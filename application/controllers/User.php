...
class User extends CI_Controller
{
    ...
    public function anggota()
    {
        ...
        $this->load->view('backend/templates/sidebar', $data);
        $this->load->view('backend/templates/topbar', $data);
        $this->load->view('backend/user/anggota', $data);
        $this->load->view('backend/templates/footer');
    }

    public function ubahProfil()
    {
        $data['nama'] = 'Ubah Profil';
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('backend/templates/sidebar', $data);
            $this->load->view('backend/templates/topbar', $data);
            $this->load->view('backend/user/ubah-profile', $data);
            $this->load->view('backend/templates/footer');
        } else {
            ...
            if ($upload_image) {
                ...
                if ($this->upload->do_upload('image')) {
                    ...
                    if ($gambar_lama != 'default.jpg') {
                        unlink(FCPATH . 'assets/backend/img/profile/' . $gambar_lama);
                    }
    ...
    public function ubahPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('backend/templates/sidebar', $data);
            $this->load->view('backend/templates/topbar', $data);
            $this->load->view('backend/user/ubah-password', $data);
            $this->load->view('backend/templates/footer');
        } else {
            ...
            if (!password_verify($pwd_skrg, $data['user']['password'])) {
                ...
                redirect('backend/user/ubahPassword');
            } else {
